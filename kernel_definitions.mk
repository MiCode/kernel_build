...
TARGET_KERNEL_MAKE_ENV += WT_FINAL_RELEASE=yes
endif
ifeq ($(strip $(WT_COMPILE_FACTORY_VERSION)),yes)
TARGET_KERNEL_MAKE_ENV += WT_COMPILE_FACTORY_VERSION=yes
endif
...
ifeq ($(TARGET_BUILD_VARIANT), user)
        $(warning ###enable kernel config ENABLE_MIUI_DEBUGGING###)
        ifeq ($(ENABLE_MIUI_DEBUGGING), true)
        $(warning ###enable kernel ENABLE_MIUI_DEBUGGING###)
               TARGET_KERNEL_MAKE_ENV_TMP += CONFIG_DEBUG_FS=y
        endif
endif
...
ifeq (jp, $(strip $(MIUI_BUILD_SUPPORT_REGION)))
TARGET_KERNEL_MAKE_ENV += WT_COMPILE_JP_CHARGE=yes
endif
...
KERNEL_CONFIG_OVERRIDE += CONFIG_ECRYPT_FS=n
KERNEL_CONFIG_OVERRIDE += CONFIG_ENCRYPTED_KEYS=n

KERNEL_CONFIG_OVERRIDE += CONFIG_QCOM_RTB_SEPARATE_CPUS=n
KERNEL_CONFIG_OVERRIDE += CONFIG_QCOM_RTB=n
KERNEL_CONFIG_OVERRIDE += CONFIG_QCOM_RTB_QGKI=n
else
...
#Build region is China or not
ifneq (true,$(PRODUCT_BUILD_INTERNATIONAL))
KERNEL_CONFIG_OVERRIDE_REGION := CONFIG_REGION_IS_CN=y
endif
...
$(info "FINAL GKI_KERNEL IS:" $(GKI_KERNEL))
...
$(info "FINAL GKI_PLATFORM_NAME IS:" $(GKI_PLATFORM_NAME))
...
$(info "FINAL TARGET_USES_UNCOMPRESSED_KERNEL IS:" $(TARGET_USES_UNCOMPRESSED_KERNEL))
...
define build-kernel
	...
	VENDOR_RAMDISK_KERNEL_MODULES="$(VENDOR_RAMDISK_KERNEL_MODULES)" \
        KERNEL_CONFIG_OVERRIDE_REGION=$(KERNEL_CONFIG_OVERRIDE_REGION) \
	TARGET_PRODUCT=$(TARGET_BOARD_PLATFORM) \
	KERNEL_CONFIG_OVERRIDE="$(KERNEL_CONFIG_OVERRIDE)" \
	device/qcom/kernelscripts/buildkernel.sh \
	$(real_cc) \
	$(TARGET_KERNEL_MAKE_ENV)\
	$(TARGET_KERNEL_MAKE_ENV_TMP)
endef

define build-kernel-gki
        KERNEL_DIR=$(TARGET_KERNEL_SOURCE) \
        DEFCONFIG=$(1) \
        OUT_DIR=$(2) \
        MAKE_PATH=$(MAKE_PATH)\
...
$(GKI_TARGET_PREBUILT_KERNEL): $(DTC) $(UFDT_APPLY_OVERLAY) $(GKI_KERNEL_OUT)
	...
	$(call build-kernel-gki,$(GKI_KERNEL_DEFCONFIG),$(GKI_KERNEL_OUT),$(GKI_KERNEL_MODULES_OUT),$(GKI_KERNEL_HEADERS_INSTALL),0,$(GKI_TARGET_PREBUILT_INT_KERNEL))
