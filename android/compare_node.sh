...
STANDARD_NODE_FILE="$ROOT_DIR/qcom/opensource/devicetree/reserved_memory_control/"$2"_standard_node_info.txt"  #standard_node_info.txt的绝对路径
...
# 检查标准节点文件存放的路径
# NOTE: 每个平台devicetree的路径可能存在变化
if [ ! -e ${STANDARD_NODE_FILE} ];then
	echo "${STANDARD_NODE_FILE} does not exist, please check if your specific path correct. Exiting ..."
	exit 1
fi
