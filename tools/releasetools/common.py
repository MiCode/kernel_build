  ...
  with zipfile.ZipFile(filename, allowZip64=True) as input_zip:
    ...
    if patterns is not None:
      for pattern in patterns:
        matched_names = fnmatch.filter(names, pattern)
        if matched_names:
          filtered.extend(matched_names)
    else:
      filtered = names
    ...
    for filename in filtered:
      info = input_zip.getinfo(filename)
      ...
      if stat.S_ISDIR(attr):
        ...
        os.symlink(input_zip.read(filename), output_file)
      else:
        with open(output_file, 'wb') as output:
          output.write(input_zip.read(filename))
        if mode != 0:
          os.chmod(output_file, mode)
...
def ZipDelete(zip_filename, entries):
  ...
  """
  with zipfile.ZipFile(zip_filename, allowZip64=True) as input_zip:
    names = input_zip.namelist()

  target_file_tmp = zip_filename + '.tmp'
  OPTIONS.tempfiles.append(target_file_tmp)

  input_zip = zipfile.ZipFile(zip_filename, "r", allowZip64=True)
  output_zip = zipfile.ZipFile(target_file_tmp, "w", compression=zipfile.ZIP_DEFLATED, allowZip64=True)
  ...
  for info in input_zip.infolist():
    ...
    ZipWriteStr(output_zip, out_info, data)

  ZipClose(input_zip)
  ZipClose(output_zip)

  OPTIONS.tempfiles.remove(target_file_tmp)
  os.rename(target_file_tmp, zip_filename)
