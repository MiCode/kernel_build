...
def download_apks(output_dir, apk_info_url, device_name, build_profile, build_region, build_android_codebase, new_apkcerts_dir):
    ...
    return userdata_download_dir


filesNum = 0

def countFileNum(input_dir):
    global filesNum
    if os.path.exists(input_dir) and os.listdir(input_dir):
        for sub_dir in os.listdir(input_dir):
            ...
            if os.path.isfile(sub_path):
                ...
                countFileNum(sub_path)

def generate_target_files_zip(userdata_download_dir, target_files_zip):
    countFileNum(userdata_download_dir)
    if filesNum > 0:
        out_dir = os.path.dirname(target_files_zip)
        if not os.path.exists(out_dir):
            run_cmd(f"mkdir -p {out_dir}")
        run_cmd(f"mv {userdata_download_dir} {out_dir}/DATA && cd {out_dir}/ && zip -r {target_files_zip} ./DATA && rm -r {out_dir}/DATA && cd - ")
...
def extract_userdata_target_files(output_dir, mi_addon_misc_info):
    if not os.path.exists(mi_addon_misc_info):
        return
    mi_addon_misc_dict = common.LoadDictionaryFromFile(mi_addon_misc_info)
    userdata_target_files_zip = mi_addon_misc_dict.get("userdata_target_files_zip")
    new_apkcerts_dir = mi_addon_misc_dict.get("new_apkcerts_dir")
    input_merge_output_dir = mi_addon_misc_dict.get("input_merge_output_dir")
    if userdata_target_files_zip is not None and os.path.exists(userdata_target_files_zip):
        #unzip the zip
        userdata_download_dir = f"{output_dir}/userdata_download_dir"
        run_cmd(f"mkdir -p {userdata_download_dir}")
        run_cmd(f"unzip {userdata_target_files_zip} -d {userdata_download_dir}")
        if os.listdir(f'{userdata_download_dir}/DATA'):
            PANGU_REMAKE_SUCCESS_FLAG = os.path.join(input_merge_output_dir, ".pangu_userdata_remake_success.lock")
            del_file_if_exist(PANGU_REMAKE_SUCCESS_FLAG)
            logger.info("start preparing userdata image")
            raw_apk_certs = os.path.join(output_dir, "META", "apkcerts.txt")
            ...
            update_miscinfo_txt(f"{output_dir}/META")
            run_cmd(f"cp -a {userdata_download_dir}/DATA/. {output_dir}/DATA/")
            run_cmd(f"rm -r {userdata_download_dir}")
            run_cmd(f"rm -r {new_apkcerts_dir}")
            # create an empty file to notify CI that userdata image has been remaking successfully.
            try:
                open(PANGU_REMAKE_SUCCESS_FLAG, 'w+')
                run_cmd(f"ls -la {PANGU_REMAKE_SUCCESS_FLAG}")
            except OSError as e:
                raise Exception(
...
def main():
    ...
    parser.add_argument('--target-files-zip', nargs='?', type=str)
    parser.add_argument('--build-phrase', nargs='?', type=str)
    parser.add_argument('--device-name', nargs='?', type=str)
    parser.add_argument('--build-profile', nargs='?', type=str)
    parser.add_argument('--mi-addon-misc-info', nargs='?', type=str)
    ...
    parser.add_argument('--new-apkcerts-dir', nargs='?', type=str)
    args = parser.parse_args()

    logger.info(f"args.output_dir = {args.output_dir}, args.target_files_zip = {args.target_files_zip}, args.mi_addon_misc_info = {args.mi_addon_misc_info}, "
                f"args.device_name = {args.device_name}, args.build_profile = {args.build_profile}, args.new_apkcerts_dir = {args.new_apkcerts_dir}, "
                f"args.build_region = {args.build_region}, args.build_android_codebase = {args.build_android_codebase}, "
                f" args.apk_info_url = {args.apk_info_url}, args.build_phrase = {args.build_phrase}")
    ...
    mi_addon_misc_info = args.mi_addon_misc_info
    new_apkcerts_dir = args.new_apkcerts_dir
    global bundletool
    ...
    if args.build_phrase == "download":
        if apk_info_url and apk_info_url != 'None' and apk_info_url.upper() != 'NULL':
            userdata_download_dir = download_apks(output_dir, apk_info_url, device_name, build_profile,
                                                                build_region, build_android_codebase, new_apkcerts_dir)
            generate_target_files_zip(userdata_download_dir, args.target_files_zip)
    elif args.build_phrase == "build":
        extract_userdata_target_files(output_dir, mi_addon_misc_info)
