...
"""
def combine_notice_files_html(draft_oss_written_offer,file_hash, input_dir, output_filename):
    """Combine notice files in FILE_HASH and output a HTML version to OUTPUT_FILENAME."""
    SRC_DIR_STRIP_RE = re.compile(input_dir + "(/.*).txt")
    # Set up a filename to row id table (anchors inside tables don't work in
    # most browsers, but href's to table row ids do)
    id_table = {}
    offer_title = draft_oss_written_offer["offer_title"]
    id_table[offer_title] = 0
    id_count = 1
    ...
    print >> output_file, '<tr><td class="same-license">'
    print >> output_file
    print >> output_file, '<div class="file-list">'
    print >> output_file, "%s <br/>" % (offer_title)
    print >> output_file, "</div><!-- file-list -->"
    print >> output_file, '<pre class="license-text">'
    print >> output_file, html_escape(open(draft_oss_written_offer["file_offer"]).read())
    print >> output_file, "</pre><!-- license-text -->"
    print >> output_file, "</td></tr><!-- same-license -->"
    print >> output_file, "</div><!-- table of offer -->"
    print >> output_file, '<div class="toc">'
    ...
    output_file.close()
def combine_notice_files_text(draft_oss_written_offer,file_hash, input_dir, output_filename, file_title):
    """Combine notice files in FILE_HASH and output a text version to OUTPUT_FILENAME."""
    SRC_DIR_STRIP_RE = re.compile(input_dir + "(/.*).txt")
    output_file = open(output_filename, "wb")
    print >> output_file, file_title
    print >> output_file, "============================================================"
    print >> output_file, draft_oss_written_offer["offer_title"]
    print >> output_file, "------------------------------------------------------------"
    print >> output_file, open(draft_oss_written_offer["file_offer"]).read()
    for value in file_hash:
      print >> output_file, "============================================================"
    ...
    output_file.close()
def combine_notice_files_xml(draft_oss_written_offer,files_with_same_hash, input_dir, output_filename):
    ...
    id_table = {}
    offer_title = draft_oss_written_offer["offer_title"]
    file_offer = draft_oss_written_offer["file_offer"]
    ...
    print >> output_file, '<file-name contentId="%s">%s</file-name>' % (offer_title, offer_title)
    # Print out a nice table of contents
    for filename in sorted_filenames:
        stripped_filename = SRC_DIR_STRIP_RE.sub(r"\1", filename)
    ...
    processed_file_keys = []
    # Output the individual notice file lists
    print >> output_file, '<file-content contentId="%s"><![CDATA[%s]]></file-content>' % (
    offer_title, html_escape(open(file_offer).read()))
    print >> output_file
    for filename in sorted_filenames:
        ...
        print >> output_file, '<file-content contentId="%s"><![CDATA[%s]]></file-content>' % (
        file_key, html_escape(open(filename).read()))
        print >> output_file
    # Finish off the file output
    print >> output_file, "</licenses>"
    output_file.close()
    ...
    for root, dir, files in os.walk(input_dir):
        for file in files:
            ...
            if matched and file.endswith(".txt"):
                ...
                # Find GNU GENERAL PUBLIC LICENSE and GNU LESSER GENERAL PUBLIC LICENSE and md5 them
                try:
                    openfile = open(os.path.join(filename), "r")
                    lines = openfile.readlines()
                except IOError:
                    lines = ""
                for line in lines:
                    if line.find("GNU GENERAL PUBLIC LICENSE") != -1:
                        file_md5sum = md5sum(filename)
                        files_with_same_hash[file_md5sum].append(filename)
                        break
                    elif line.find("GNU LESSER GENERAL PUBLIC LICENSE") != -1:
                        file_md5sum = md5sum(filename)
                        files_with_same_hash[file_md5sum].append(filename)
                        break
    offer_title = "Draft OSS Written Offer"
    pwd = "build/make/tools"
    file_offer = os.path.join(pwd,"Draft_OSS_Written_Offer")
    draft_oss_written_offer["offer_title"] = offer_title
    draft_oss_written_offer["file_offer"] = file_offer
    filesets = [sorted(files_with_same_hash[md5]) for md5 in sorted(files_with_same_hash.keys())]
    combine_notice_files_text(draft_oss_written_offer,filesets, input_dir, txt_output_file, file_title)
    if html_output_file is not None:
        combine_notice_files_html(draft_oss_written_offer,filesets, input_dir, html_output_file)
    if xml_output_file is not None:
        combine_notice_files_xml(draft_oss_written_offer,files_with_same_hash, input_dir, xml_output_file)
