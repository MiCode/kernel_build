...
"""Incoming edge transition for `modules_prepare`."""

load(":abi/trim_nonlisted_kmi_utils.bzl", "trim_nonlisted_kmi_utils")

def _modules_prepare_transition_impl(settings, attr):
    ret = trim_nonlisted_kmi_utils.transition_impl(settings, attr)
    return ret

modules_prepare_transition = transition(
    implementation = _modules_prepare_transition_impl,
