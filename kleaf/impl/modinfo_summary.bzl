...
def _modinfo_summary_report_impl(ctx):
    ...
    modules_staging_archive = ctx.attr.kernel_build[KernelBuildExtModuleInfo].modules_staging_archive
    ...
    command = hermetic_tools.setup + """
        ...
        # Run the reporter
        {modinfo_summary} --arch {quoted_arch} --branch {quoted_branch} \\
          --directory {intermediates_dir} --output {modinfo_summary_xml}
    ...
    """.format(
        ...
        quoted_branch = shell.quote(ctx.attr.branch),
    )
