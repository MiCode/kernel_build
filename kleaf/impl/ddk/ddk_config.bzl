...
def _create_kconfig_ext_step(ctx, kconfig_depset_file):
    ...
    cmd = """
        ...
        rsync --no-group -L -r --include="*/" --include="Kconfig*" --exclude="*" ${{KERNEL_DIR}}/${{KCONFIG_EXT_PREFIX}} {intermediates_dir}/
...
def _create_main_action(
    ...
    command += """
        ...
        # Copy outputs
        rsync --no-group -L ${{OUT_DIR}}/.config {out_dir}/.config
        rsync --no-group -L -r ${{OUT_DIR}}/include/ {out_dir}/include/
    """.format(
...
def _create_env_and_outputs_info(ctx, out_dir):
    ...
    restore_outputs_cmd = """
        rsync --no-group -L {out_dir}/.config ${{OUT_DIR}}/.config
        rsync --no-group -L -r --chmod=D+w {out_dir}/include/ ${{OUT_DIR}}/include/
    """.format(
        out_dir = out_dir.path,
    )
