...
def _dump_modules_abi_subrule_impl(
    ...
    modules_abi = subrule_ctx.actions.declare_file("{}/abi.stg".format(subrule_ctx.label.name))

    # Find the module
    modules = "$(find {dir_path} -name '*.ko')".format(
        dir_path = unstripped_dir.path,
    )
    command = """
        # Clean PATH to avoid using non-hermetic tools.
        export PATH=
        {stg} --files '*.h' --output {modules_abi} --elf {modules}
    """.format(
        stg = _stg.executable.path,
        modules_abi = modules_abi.path,
        modules = modules,
    )
